...
class SemgrepCLIService:
    ...
    def __init__(self, ruleset='p/ci'):
        ...
        try:
            subprocess.run(['semgrep', '--version'], capture_output=True, text=True, timeout=5)
    ...
    def _detect_language(self, code: str) -> str:
        ...
        # Python
        if any(k in code_lower for k in ['def ', 'import ', 'from ', 'print(', 'class ', '__init__']):
            if not any(k in code_lower for k in ['function ', 'var ', 'let ', 'const ', '=>']):
                return 'python'
        
        # JavaScript
        if any(k in code_lower for k in ['function ', 'var ', 'let ', 'const ', '=>', 'console.log', 'document.']):
            if 'interface ' not in code_lower and 'type ' not in code_lower:
                return 'javascript'
        
        # TypeScript
        if any(k in code_lower for k in ['interface ', 'type ', ': string', ': number', ': boolean']):
            return 'typescript'
        
        # Java
        if any(k in code_lower for k in ['public class', 'public static void', 'import java.', 'package java', '@override']):
            return 'java'
        
        # C/C++
        if any(k in code_lower for k in ['#include', 'int main', 'printf', 'cout', 'std::', 'namespace ']):
            return 'cpp' if any(k in code_lower for k in ['cout', 'std::', 'namespace ', 'class ', 'template<']) else 'c'
        
        # Go
        if any(k in code_lower for k in ['package ', 'func ', 'import "', ':= ', 'go func']):
            return 'go'
        
        # Ruby
        if any(k in code_lower for k in ['def ', 'end', 'require ', 'class ', 'module ']) and 'def ' in code_lower:
            if 'end' in code_lower and 'print(' not in code_lower:
                return 'ruby'
        
        # PHP
        if any(k in code_lower for k in ['<?php', '$_', '->', 'function ', 'class ', 'namespace ']):
            if '<?php' in code_lower or '$' in code_lower[:100]:
                return 'php'
        
        # C#
        if any(k in code_lower for k in ['using system', 'namespace ', 'public class', 'private ', 'get;', 'set;']):
            if 'using system' in code_lower or 'namespace ' in code_lower:
                return 'csharp'
        
        # Scala
        if any(k in code_lower for k in ['object ', 'def ', 'val ', 'var ', 'def ', 'extends ', 'trait ']):
            if 'object ' in code_lower and 'extends ' in code_lower:
                return 'scala'
        
        # Kotlin
        if any(k in code_lower for k in ['fun ', 'val ', 'var ', 'class ', 'data class', 'companion object']):
            if 'fun ' in code_lower:
                return 'kotlin'
        
        # Rust
        if any(k in code_lower for k in ['fn ', 'let ', 'mut ', 'struct ', 'impl ', 'use ', '::']):
            if 'fn ' in code_lower and '::' in code_lower:
                return 'rust'
        
        # Swift
        if any(k in code_lower for k in ['func ', 'var ', 'let ', 'class ', 'struct ', 'import swift', 'guard ']):
            if 'func ' in code_lower and 'import swift' in code_lower:
                return 'swift'
        
        # Lua
        if any(k in code_lower for k in ['function ', 'local ', 'end', 'require(', 'print(']):
            if 'local ' in code_lower and 'end' in code_lower:
                return 'lua'
        
        # OCaml
        if any(k in code_lower for k in ['let ', 'in ', 'match ', 'with ', 'type ', 'module ']):
            if 'let ' in code_lower and 'in ' in code_lower:
                return 'ocaml'
        
        # Terraform
        if any(k in code_lower for k in ['resource ', 'provider ', 'variable ', 'output ', 'terraform {']):
            return 'terraform'
        
        # YAML
        if any(k in code_lower for k in ['---', ':', 'apiVersion:', 'kind:', 'metadata:']):
        ...
        # HTML
        if any(k in code_lower for k in ['<!doctype', '<html', '<head', '<body', '<div', '<script']):
            return 'html'
        
        # Dockerfile
        if any(k in code_lower for k in ['from ', 'run ', 'copy ', 'workdir ', 'expose ', 'cmd ']):
            if code_lower.startswith('from ') or 'from ' in code_lower[:50]:
                return 'dockerfile'
        
        # Bash/Shell
        if any(k in code_lower for k in ['#!/bin/bash', '#!/bin/sh', 'echo ', 'export ', 'if [', 'then ']):
            if code_lower.startswith('#!') or 'if [' in code_lower:
                return 'bash'
        
        # Apex (Salesforce)
        if any(k in code_lower for k in ['public class', 'trigger ', '@istest', 'database.', 'sobject ']):
            if 'trigger ' in code_lower or '@istest' in code_lower:
                return 'apex'
        
        # Clojure
        if any(k in code_lower for k in ['(def ', '(defn ', '(let ', '(if ', '(fn ', 'ns ']):
            if '(def ' in code_lower or '(defn ' in code_lower:
                return 'clojure'
        
        # Dart
        if any(k in code_lower for k in ['void main()', 'class ', 'import ', 'dart:', 'async ', 'await ']):
            if 'void main()' in code_lower or 'dart:' in code_lower:
                return 'dart'
        
        # Elixir
        if any(k in code_lower for k in ['defmodule ', 'def ', 'defp ', 'defmacro ', '|>', 'do: ']):
            if 'defmodule ' in code_lower or '|>' in code_lower:
                return 'elixir'
        
        # JSX
        if any(k in code_lower for k in ['<div', '<component', 'react.', 'import react', 'jsx', 'return (']):
            if '<div' in code_lower or 'react.' in code_lower:
                if 'interface ' not in code_lower and ': string' not in code_lower:
                    return 'jsx'
        
        # Julia
        if any(k in code_lower for k in ['function ', 'end', 'using ', 'import ', '::', 'println(']):
            if 'function ' in code_lower and '::' in code_lower:
                return 'julia'
        
        # Jsonnet
        if any(k in code_lower for k in ['local ', 'function(', 'self.', 'super.', 'std.']):
            if 'local ' in code_lower and 'std.' in code_lower:
                return 'jsonnet'
        
        # Lisp
        if any(k in code_lower for k in ['(defun ', '(defvar ', '(setq ', '(if ', '(let ', '(lambda ']):
            if '(defun ' in code_lower or '(lambda ' in code_lower:
                return 'lisp'
        
        # R
        if any(k in code_lower for k in ['<-', 'function(', 'library(', 'data.frame', 'ggplot2', 'print(']):
            if '<-' in code_lower and 'function(' in code_lower:
                return 'r'
        
        # Scheme
        if any(k in code_lower for k in ['(define ', '(lambda ', '(let ', '(if ', '(cond ', '(car ', '(cdr ']):
            if '(define ' in code_lower and '(lambda ' in code_lower:
                return 'scheme'
        
        # Solidity
        if any(k in code_lower for k in ['pragma solidity', 'contract ', 'function ', 'modifier ', 'event ', 'mapping(']):
            if 'pragma solidity' in code_lower or 'contract ' in code_lower:
                return 'solidity'
        
        # TSX
        if any(k in code_lower for k in ['<div', '<component', 'react.', 'import react', 'interface ', ': string']):
    ...
    def _run_code_scan(self, code: str) -> dict:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # Для Dockerfile используем имя файла без расширения
            ...
            if result.returncode != 0:
                raise RuntimeError(f'Ошибка сканирования: {result.stderr or result.stdout}')
    ...
    def _run_repo_scan(self, repo_url: str) -> dict:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if clone.returncode != 0:
                raise RuntimeError(f'Ошибка клонирования: {clone.stderr or clone.stdout}')
