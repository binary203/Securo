...
class SemgrepCLIService:
    ...
    def _run_file_scan(self, file_path: str) -> dict:
        ...
        file_ext = os.path.splittext(file_path)[1]
        file_name = os.path.basename(file_path)

        Detected_language = None
        for lang, ext in self.Language.items():
            if file_ext == ext or (file_name == "dockerfile" or lang == "dockerfile"):
        ...
        if not Detected_language:
            try:
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    code_content = f.read()
                detected_langiage = self._detect_language(code_content)
            except:
                raise RuntimeError("Не удалось определить язык")

        file_scan = subprocess.run(
            ["semgrep", "--config", self.ruleset, "q", "--json", file_path]
        )
        ...
        if file_scan.returncode != 0:
            ...
            raise RuntimeError(f"язык: {Detected_language}, файл: {file_path}: {error}")

        try:
            return json.loads(file_scan.stdout)
        except json.JSONDecodeError:
            raise RuntimeError(f"Ошибк апарсинга рузультата: {file_scan.stdout[:200]}")
